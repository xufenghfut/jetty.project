...
//                            ((HTTP2Session)session).newStreamOld(requestFrame, promise, new Stream.Listener.Adapter());
        ...
        Assert.assertTrue(String.format("Missing streams on client: %d/%d", clientLatch.getCount(), total), clientLatch.await(total, TimeUnit.MILLISECONDS));
        long elapsed = System.nanoTime() - start;
        System.err.println("elapsed = " + elapsed);
        Assert.assertTrue(String.format("Missing streams on server: %d/%d", serverLatch.getCount(), total), serverLatch.await(total, TimeUnit.MILLISECONDS));
    }
}
